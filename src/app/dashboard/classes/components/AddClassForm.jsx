...
export default function AddClassForm({ open, onOpenChange, onSuccess }) {
  ...
  const disableSubmit = useMemo(() => {
    return submitting || loadingTeachers || teacherOptions.length === 0;
  }, [submitting, loadingTeachers, teacherOptions.length]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.name || !form.teacherId) {
      setFormError("Class name and teacher are required.");
      return;
    }
    setSubmitting(true);
    setFormError("");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Add a class and assign a teacher.</DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <Button type="submit" className="w-full" disabled={disableSubmit}>
            {submitting ? "Saving..." : loadingTeachers ? "Loading teachers..." : "Save"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
